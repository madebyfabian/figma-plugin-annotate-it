type Data = {
  newAnnots: Annotation[], 
  oldAnnots: Annotation[]
}
...
export default ( activeWrapperId: string, data: Data ) => {
  let annotWrapperNode = getAnnotWrapperNodeById(activeWrapperId)
  if (!annotWrapperNode)
    return console.error('No annotation wrapper node exists. fix this!')

  const { newAnnots, oldAnnots } = data,
